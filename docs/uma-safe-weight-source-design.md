...
- DeciLM/Nemotron NAS uses the shared dense hook while preserving its no-op-layer
  model, tied `lm_head` skip, and qkv/gate-up packed mapper
- Mistral3 uses the shared dense hook for its multimodal language/vision/projector
...
- AXK1 MoE
  - reuses the DeepSeek-style WeightSource hook for routed experts, packed
    attention/MLP mappings, and speculative-layer skip
...
- GLM4 MoE
  - initial hook implemented for per-expert
...
- Arctic MoE
  - initial hook implemented for the model's child-loader qkv, dense MLP,
    residual MLP, and local expert mappings
  - reads TP-local expert slices for `experts.<id>.w{1,2,3}.weight` before
...
- Ernie 4.5 MoE
  - initial hook implemented for standard
    `mlp.experts.<expert>.{gate,up,down}_proj.*` tensors
  - skips non-local routed experts before payload read
  - preserves `moe_statics` to `gate` bias remapping, tied `lm_head` skip, and
    MTP tensor skip in the model-side plan
- Bailing-style MoE
  - initial hook implemented for standard
    `mlp.experts.<expert>.{gate,up,down}_proj.*` tensors
  - skips non-local routed experts before payload read
...
- MiniCPM / EagleMiniCPM MoE
  ...
  - preserves qkv/gate-up packed dense mappings, rotary-cache skips, tied
    `lm_head` skip, and Eagle own-head/embed detection in the model-side plan
