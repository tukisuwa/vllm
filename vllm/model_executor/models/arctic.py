...
def _arctic_moe_layer(config: ArcticConfig, layer: int) -> bool:
    return (layer + 1) % config.moe_layer_frequency == 0


def _arctic_model_weight_plan(
    model: nn.Module,
    ...
    *,
    ...
    for checkpoint_name in catalog.names():
        ...
        for param_name, shard_name, shard_id in (
            ...
            if shard_name in name:
                entry = WeightPlanEntry(
                    ...
                    target_name=target_prefix + name.replace(shard_name, param_name),
                    shard_id=shard_id,
        ...
        for layer in range(config.num_hidden_layers):
            is_moe_layer = _arctic_moe_layer(config, layer)
            if is_moe_layer and config.use_residual:
            ...
            if is_moe_layer:
                layer_prefix = f"layers.{layer}.block_sparse_moe."
...
def _arctic_resolve_attr(root: object, path: str) -> object:
    current = root
    for part in path.split("."):
        if not hasattr(current, part):
            raise RuntimeError(f"Cannot resolve Arctic UMA target {path!r}")
        current = getattr(current, part)
    return current


def _arctic_load_weights_from_source(
    model: nn.Module,
    ...
    for entry in plan:
        ...
        if entry.weight_name.endswith("w1.weight"):
            ...
            raise RuntimeError(
                "Arctic UMA expert entry has unknown weight name: "
                f"{entry.weight_name}"
            )
...
class ArcticForCausalLM(nn.Module, SupportsPP, SupportsQuant):
    ...
    def build_weight_plan(self, catalog: TensorCatalog) -> WeightPlan:
        plan = _arctic_model_weight_plan(self.model, catalog)
    ...
    def load_weights_from_source(
        ...
        return _arctic_load_weights_from_source(self, source, plan)
